...
export class Fn extends Component {
     constructor(){
        super()
        this.state ={
            mobiles : [{
     ...
     ascending = () => {
        ...
        let fObj = a.name.toLowerCase();
        let sObj = b.name.toLowerCase();
        if(fObj < sObj){
            return -1;
        }else if(fObj > sObj){
            return 1;
...
<button onClick={() => this.ascendingPrice()}>AscendingPrice</button>
<button onClick={() => this.decendingPrice()}>DescendingPrice</button>
